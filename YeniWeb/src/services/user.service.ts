...
export const userService = {
  async list(): Promise<User[]> {
    const response = await apiService.get<User[] | ApiEnvelope<User[]>>('/users');
    return unwrap(response);
  },

  async create(payload: CreateUserRequest): Promise<User> {
    const response = await apiService.post<User | ApiEnvelope<User>>('/users', payload);
    return unwrap(response);
  },

  async update(id: string, payload: UpdateUserRequest): Promise<User> {
    const response = await apiService.put<User | ApiEnvelope<User>>(`/users/${id}`, payload);
    return unwrap(response);
  },
  ...
  async setActive(id: string, isActive: boolean): Promise<User> {
    const response = await apiService.post<User | ApiEnvelope<User>>(`/users/${id}/status`, { isActive });
    return unwrap(response);
  },
};
