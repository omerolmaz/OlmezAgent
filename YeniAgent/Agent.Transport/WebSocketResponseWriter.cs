...
public sealed class WebSocketResponseWriter : IAgentResponseWriter
{
    ...
    public async Task SendAsync(CommandResult result, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // For initial handshake messages (register, heartbeat), send as-is with their action
            if (result.Action == "register" || result.Action == "heartbeat" || result.Action == "agenthello")
            {
                responseEnvelope = result.Payload;
            }
            else
            {
                // For command results (including agentinfo), wrap in commandresult envelope
