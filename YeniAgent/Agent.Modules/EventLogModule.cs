...
public sealed class EventLogModule : AgentModuleBase
{
    ...
    private async Task HandleGetEventLogsAsync(AgentCommand command, AgentContext context)
    {
        ...
        if (command.Payload.ValueKind == JsonValueKind.Undefined || command.Payload.ValueKind == JsonValueKind.Null)
        {
            ...
            var defaultEvents = await Task.Run(() => GetEventLogs("Application", 100, null, 24)).ConfigureAwait(false);
            ...
            await context.ResponseWriter.SendAsync(new CommandResult(
                ...
                command.NodeId,
                command.SessionId,
                new JsonObject { ["events"] = JsonSerializer.SerializeToNode(defaultEvents) },
